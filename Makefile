...
else ifeq (rocksdb,$(findstring rocksdb,$(FANX_BUILD_OPTIONS)))
...
install: enforce-go-version
	@echo "🤖 Installing fanx..."
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/fanx
	@echo "✅ Completed installation!"

build: enforce-go-version
	@echo "🤖 Building fanx..."
	go build $(BUILD_FLAGS) -o $(BUILDDIR)/ ./cmd/fanx
...
build-reproducible-amd64: go.sum $(BUILDDIR)/
	...
	$(DOCKER) create -ti --name fanxbinary fanx:local-amd64
	$(DOCKER) cp fanxbinary:/bin/fanx $(BUILDDIR)/fanx-linux-amd64
...
containerProtoGenGo=fanx-proto-gen-go-$(protoVer)
containerProtoGenSwagger=fanx-proto-gen-swagger-$(protoVer)
