...
else ifeq (rocksdb,$(findstring rocksdb,$(FURYX_BUILD_OPTIONS)))
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=fuyrx \
          -X github.com/cosmos/cosmos-sdk/version.AppName=fuyrxd \
...
install: enforce-go-version
	@echo "🤖 Installing fuyrxd..."
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/fuyrxd
	@echo "✅ Completed installation!"

build: enforce-go-version
	@echo "🤖 Building fuyrxd..."
	go build $(BUILD_FLAGS) -o $(BUILDDIR)/ ./cmd/fuyrxd
...
build-reproducible-amd64: go.sum $(BUILDDIR)/
	$(DOCKER) buildx create --name fuyrxbuilder || true
	$(DOCKER) buildx use fuyrxbuilder
	$(DOCKER) buildx build \
		...
		--load \
		-f Dockerfile .
	$(DOCKER) rm -f fuyrxbinary || true
	$(DOCKER) create -ti --name fuyrxbinary fuyrx:local-amd64
	$(DOCKER) cp fuyrxbinary:/bin/fuyrxd $(BUILDDIR)/fuyrxd-linux-amd64
	$(DOCKER) rm -f fuyrxbinary
...
build-reproducible-arm64: go.sum $(BUILDDIR)/
	...
	$(DOCKER) buildx build \
		...
		-f Dockerfile .
	$(DOCKER) rm -f fuyrxbinary || true
	$(DOCKER) create -ti --name fuyrxbinary fuyrx:local-arm64
	$(DOCKER) cp fuyrxbinary:/bin/fuyrxd $(BUILDDIR)/fuyrxd-linux-arm64
	$(DOCKER) rm -f fuyrxbinary
...
containerProtoGenGo=fuyrx-proto-gen-go-$(protoVer)
containerProtoGenSwagger=fuyrx-proto-gen-swagger-$(protoVer)
